  ...
  useEffect(() => {
    if (grade === 'all') {  // 학년이 전체라면 ?
      if (room === 'all') { // 반이 전체일 경우?
        setStudents(students) //[ 학년전체& 반전체] 그대로 둔다 
      } 
      ...
      if (room === 'all') { // [ N학년& 전체반]
        ...
        setStudents(everyStudent.filter(student => student.room.roomNum === Number(grade) && student.room.roomNum === room))
      }
    }
  }, [room, grade])
    ...
    <div className='card-case'>
      <div className='grade_room_box'>
        <div className='grade_select'>
          <p className='grade-text' onClick={() => setGrade('all')}><span className='line'>전체</span></p>
          ...
          {grade==='all'?
          <Button id={room} color="white" colorScheme="cyan" className='myroom_select'> 우리반보기 </Button>:
          <Button id={room} color="white" colorScheme="cyan" className='myroom_select2'> 우리반보기 </Button>}
          {grade!=='all'&&
