 ...
 */
@Api(value = "인증 API", tags = {"Autht."})
@RestController
@RequestMapping("/api/v1/authTeacher")
public class AuthTeacherController {
	...
	public ResponseEntity<TeacherLoginPostRes> login(@RequestBody @ApiParam(value="로그인 정보", required = true) TeacherLoginPostReq loginInfo) {
		String userId = loginInfo.getId();
		String password = loginInfo.getPassword();
		
		Teacher teacher = userService.getTeacherByTeacherId(userId);
		// 로그인 요청한 유저로부터 입력된 패스워드 와 디비에 저장된 유저의 암호화된 패스워드가 같은지 확인.(유효한 패스워드인지 여부 확인)
		if(passwordEncoder.matches(password, teacher.getTeacherPassword())) {
			// 유효한 패스워드가 맞는 경우, 로그인 성공으로 응답.(액세스 토큰을 포함하여 응답값 전달)
			return ResponseEntity.ok(TeacherLoginPostRes.of(200, "Success", JwtTokenUtil.getToken(userId)));
		}
